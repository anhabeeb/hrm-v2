...
assert(
  (refreshRoute.includes("queueOnboardingReadinessRefresh") && refreshRoute.includes("cachedOnboardingReadinessFromCase") && refreshRoute.includes("onboardingReadinessRefreshPayload")) ||
    (refreshRoute.includes("rebuildStaleOrMissingOnboardingSectionReadiness") && refreshRoute.includes("section_timings") && refreshRoute.includes("mode: \"section_status\"")),
  "manual readiness refresh does not return a tracked queued or terminal section-status refresh"
);
assert(
  (refreshRoute.includes("readiness_updating: true") && refreshRoute.includes('queued.queued ? "queued"') && refreshRoute.includes("already_queued")) ||
    (refreshRoute.includes("readiness_updating: false") && refreshRoute.includes('status: "completed"') && refreshRoute.includes("activation_requires_final_verification")),
  "manual readiness refresh does not return a safe queued/updating or completed section-status response"
);
