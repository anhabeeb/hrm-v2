...
if (!employeeId) {
  ...
  lines.push(
    ...
    `SELECT section_key, status, status_reason_code, status_message, next_action FROM employee_setup_section_statuses WHERE employee_id = '${employeeId}' AND status IN ('stale', 'failed', 'blocked') ORDER BY section_key;`,
    ...
    `SELECT e.id, e.employee_no, e.full_name, es.key AS status_key FROM employees e LEFT JOIN employee_statuses es ON es.id = e.status_id WHERE e.id = '${employeeId}' LIMIT 1;`,
