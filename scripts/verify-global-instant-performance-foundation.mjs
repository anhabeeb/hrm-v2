...
includes(api, "export const apiClient", "central typed API client is exported");
includes(api, "get<T>", "typed GET method exists");
includes(api, "post<T>", "typed POST method exists");
includes(api, "patch<T>", "typed PATCH method exists");
includes(api, "delete<T>", "typed DELETE method exists");
includes(api, "AbortSignal", "API client supports AbortSignal/cancellation");
includes(api, "API_REQUEST_TIMEOUT_MS", "API client supports timeout handling");
includes(api, "X-Request-ID", "API client sends request correlation id");
includes(api, "recordApiRequestTiming", "API client records request duration");
includes(api, "Server-Timing", "API client captures server timing");
includes(api, "hrm-v2-session-expired", "session-expired handling is preserved");
includes(api, "hrm-v2-api-permission-error", "permission response handling event exists");
includes(api, "hrm-v2-module-disabled", "module-disabled response handling event exists");
includes(api, "inflightGetRequests", "GET in-flight dedupe map exists");
includes(api, "method === \"GET\"", "dedupe is limited to GET requests");
includes(api, "dedupe !== false", "dedupe can be disabled");
includes(api, "dedupe: false", "write helpers disable dedupe");
includes(api, "if (key) inflightGetRequests.delete(key)", "failed/finished requests clear inflight map");
includes(api, "cache: method === \"GET\" ? \"no-store\"", "authenticated GETs are not public/browser cached");
includes(api, "multipartRequest", "uploads remain separate from deduped writes");
excludes(api, /method === "POST"[\s\S]{0,120}inflightGetRequests\.set/, "POST requests are not added to GET dedupe map");
