export const EMPLOYEE_SETUP_SECTION_STATUS_VALUES = [
  "not_started",
  "incomplete",
  "complete",
  "blocked",
...
export type EmployeeSetupSectionStatusValue = (typeof EMPLOYEE_SETUP_SECTION_STATUS_VALUES)[number];

export type EmployeeSetupSectionDefinition = {
  section_key: string;
...
export type EmployeeSetupSectionDefinitionContext = {
  moduleStatuses?: Record<string, boolean>;
  settings?: Record<string, unknown> | null;
  employee?: Record<string, unknown> | null;
};

export const EMPLOYEE_SETUP_SECTION_DEFINITIONS: EmployeeSetupSectionDefinition[] = [
  {
    ...
    owns_fields: ["employee_no", "full_name", "employee_type", "employment_type", "joining_date", "date_of_birth", "nationality"],
    stale_when_dependencies_change: ["employee_type", "employment_type", "joining_date", "full_name"],
  ...
  },
  {
    ...
    can_be_not_required: false,
    description: "Department, worksite/location, position, job level, and manager setup.",
    frontend_group_label: "Employee",
  ...
  },
  {
    ...
    can_be_not_required: true,
    description: "Required approvals or setup tasks before activation.",
  ...
  },
  {
    ...
    default_required: true,
    module_dependency: "employees",
    owns_fields: ["final_activation_verifier"],
    stale_when_dependencies_change: ["any_setup_section"],
    can_be_not_required: false,
    description: "Final server verification gate before activation.",
    frontend_group_label: "Activation",
    task_keys: ["final_verification"],
    setting_key: null
  }
];
...
export function isEmployeeSetupSectionModuleEnabled(definition: EmployeeSetupSectionDefinition, moduleStatuses: Record<string, boolean> = {}) {
...
export function getEmployeeSetupSectionRegistry(context: EmployeeSetupSectionDefinitionContext = {}) {
  const moduleStatuses = context.moduleStatuses ?? {};
  const settings = context.settings ?? {};
  return EMPLOYEE_SETUP_SECTION_DEFINITIONS.map((definition) => {
    const moduleEnabled = isEmployeeSetupSectionModuleEnabled(definition, moduleStatuses);
...
export function employeeSetupSectionDefinitionByKey(sectionKey: string) {
  return EMPLOYEE_SETUP_SECTION_DEFINITIONS.find((definition) => definition.section_key === sectionKey) ?? null;
}
