...
import { hasValidationErrors, validatePayrollRules, validationResponse } from "../lib/moduleValidation";
...
async function getPayslipSnapshotData(c: Context<AppBindings>, resultId: string) {
  ...
  return {
    company: { name: "Cafe Asiana", footer: "Confidential OmniCore - HR payroll document" },
    employee: {
...
payrollRoutes.post("/periods", requireAnyPermission(["payroll.periods.create", "payroll.periods.manage", "payroll.manage"]), async (c) => {
  ...
  const validationIssues = validatePayrollRules({ periodStart: startDate, periodEnd: endDate });
  if (hasValidationErrors(validationIssues)) return validationResponse(c, validationIssues);
  const settings = await getSettings(c);
...
payrollRoutes.patch("/periods/:id", requireAnyPermission(["payroll.periods.update", "payroll.periods.manage", "payroll.manage"]), async (c) => {
  ...
  const startDate = readString(body.start_date ?? old.start_date);
  const endDate = readString(body.end_date ?? old.end_date);
  const validationIssues = validatePayrollRules({ periodStart: startDate, periodEnd: endDate, status: String(old.status) });
  if (hasValidationErrors(validationIssues)) return validationResponse(c, validationIssues);
  await c.env.DB.prepare("UPDATE payroll_periods SET start_date = ?, end_date = ?, salary_payment_date = ?, updated_at = ? WHERE id = ?").bind(startDate, endDate, optionalString(body.salary_payment_date ?? old.salary_payment_date), isoNow(), id).run();
  const saved = await getPeriod(c, id);
