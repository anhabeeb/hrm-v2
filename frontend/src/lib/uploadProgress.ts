...
export function uploadWithProgress<T = unknown>(options: UploadProgressOptions) {
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    xhr.open("POST", absoluteUploadUrl(options.url));
    ...
    xhr.onerror = () => {
      recordApiRequestTiming({ method: "POST", path: options.url, status: null, durationMs: performance.now() - startedAt, requestId, cache: "network" });
      reject(new Error("Upload failed before the server could receive the file."));
    };
    xhr.onload = () => {
      recordApiRequestTiming({ method: "POST", path: options.url, status: xhr.status, durationMs: performance.now() - startedAt, requestId, cache: "network" });
