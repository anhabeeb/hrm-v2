import type { AccessScopeRule, AccessUser, AuthUser, BootstrapStatus, EmployeeUserAccessPreview, EmployeeUserAccount, Permission, Role, RoleMappingRule, UserStatus } from "../types/auth";
import type {
  ApprovalAction,
...
import { ApiError, apiClient, apiErrorFromEnvelope, blobRequest, multipartRequest, request } from "./apiClient";
import { invalidateReferenceDataCache } from "./referenceDataCache";
import { invalidateReferenceQueries } from "./queryClient";

export { API_BASE_URL, ApiError, apiClient } from "./apiClient";
...
function invalidateReferences<T>(promise: Promise<T>, prefix: string, ...extraPrefixes: string[]) {
  return promise.then((result) => {
    for (const target of [prefix, ...extraPrefixes]) {
      invalidateReferenceDataCache(target);
      invalidateReferenceQueries(target);
    }
    return result;
  });
}
...
export type OnboardingReadinessStatusResponse = {
  ...
  readiness_refresh?: {
    ...
    failed_section_key?: string | null;
    failed_section_label?: string | null;
  ...
  active_refresh?: {
    job_id?: string | null;
    status?: "queued" | "running" | "succeeded" | "failed" | "stale_failed" | string | null;
    started_at?: string | null;
    completed_at?: string | null;
    request_id?: string | null;
    last_error_code?: string | null;
...
export const api = {
  ...
  createDocumentType(token: string, input: DocumentTypeInput) {
    return invalidateReferences(request<{ document_type: DocumentType }>("/api/v1/documents/types", { method: "POST", body: JSON.stringify(input) }, token), "document");
  },
  updateDocumentType(token: string, id: string, input: DocumentTypeInput) {
    return invalidateReferences(request<{ document_type: DocumentType }>(`/api/v1/documents/types/${id}`, { method: "PATCH", body: JSON.stringify(input) }, token), "document");
  },
  documentTypeAction(token: string, id: string, action: "enable" | "disable") {
    return invalidateReferences(request<{ document_type: DocumentType }>(`/api/v1/documents/types/${id}/${action}`, { method: "POST" }, token), "document");
  },
  ...
  createDocumentRequiredRule(token: string, input: DocumentRequiredRuleInput) {
    return invalidateReferences(request<{ rule: DocumentRequiredRule }>("/api/v1/documents/required-rules", { method: "POST", body: JSON.stringify(input) }, token), "document");
  },
  updateDocumentRequiredRule(token: string, id: string, input: DocumentRequiredRuleInput) {
    return invalidateReferences(request<{ rule: DocumentRequiredRule }>(`/api/v1/documents/required-rules/${id}`, { method: "PATCH", body: JSON.stringify(input) }, token), "document");
  },
  documentRequiredRuleAction(token: string, id: string, action: "enable" | "disable") {
    return invalidateReferences(request<{ rule: DocumentRequiredRule }>(`/api/v1/documents/required-rules/${id}/${action}`, { method: "POST" }, token), "document");
  },
  ...
  createPaymentInstitution(token: string, input: Partial<PaymentInstitution>) {
    return invalidateReferences(request<{ institution: PaymentInstitution }>("/api/v1/payroll/payment-institutions", { method: "POST", body: JSON.stringify(input) }, token), "payment-institutions", "payroll");
  },
  updatePaymentInstitution(token: string, id: string, input: Partial<PaymentInstitution>) {
    return invalidateReferences(request<{ institution: PaymentInstitution }>(`/api/v1/payroll/payment-institutions/${id}`, { method: "PATCH", body: JSON.stringify(input) }, token), "payment-institutions", "payroll");
  },
  archivePaymentInstitution(token: string, id: string) {
    return invalidateReferences(request<{ archived: boolean }>(`/api/v1/payroll/payment-institutions/${id}/archive`, { method: "POST" }, token), "payment-institutions", "payroll");
  },
