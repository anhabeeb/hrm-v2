...
function PayrollTablePageLayout({ title, description, error, loading, empty, emptyTitle, filters, onReset, action, chips = [], children }: { title: string; description: string; error: string | null; loading: boolean; empty: boolean; emptyTitle: string; filters: React.ReactNode; onReset?: () => void; action?: React.ReactNode; chips?: ActiveFilterChip[]; children: React.ReactNode }) {
...
export function PayrollComponentsPage() {
  ...
  return <PayrollTablePageLayout title="Payroll Components" description="Manage Payroll Core earning and deduction components used in result line items." error={error} loading={loading} empty={filtered.length === 0} emptyTitle="No components" chips={chips} onReset={() => setSearch("")} filters={<SearchInput value={search} onChange={setSearch} placeholder="Search code/name/category" />} action={canManage ? <Button size="sm" onClick={() => setEditing({ type: "ALLOWANCE", calculation_type: "FIXED_AMOUNT", is_active: true, sort_order: 100 })}><Plus className="h-4 w-4" /> Create component</Button> : null}>
...
export function PayrollAdjustmentsPage() {
  ...
  return <PayrollTablePageLayout title="Payroll Adjustments" description="Manage manual earning and deduction adjustments." error={error} loading={loading} empty={rows.length === 0} emptyTitle="No adjustments" chips={chips} onReset={() => setSearch("")} filters={<SearchInput value={search} onChange={setSearch} />} action={canManage ? <Button size="sm" onClick={() => setEditing({ adjustment_type: "EARNING", status: "DRAFT" })}><Plus className="h-4 w-4" /> Create adjustment</Button> : null}>
