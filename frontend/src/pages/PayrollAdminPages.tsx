...
import { CheckboxField, FilterBar, PageHeader, SelectField, TextareaField } from "../components/ui/page-shell";
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<><AdminHelpLink target="payroll" label="View Payroll Guide" />{children}</>}
      />
      <PayrollNav />
    </>
  );
...
function PayrollTablePageLayout({ title, description, error, loading, empty, emptyTitle, filters, action, children }: { title: string; description: string; error: string | null; loading: boolean; empty: boolean; emptyTitle: string; filters: React.ReactNode; action?: React.ReactNode; children: React.ReactNode }) {
  return <div className="space-y-4"><PayrollPageHeader title={title} description={description}>{action}</PayrollPageHeader><ErrorMessage error={error} /><Panel className="overflow-hidden"><FilterBar className="rounded-none border-0 border-b shadow-none md:grid-cols-4 xl:grid-cols-6">{filters}</FilterBar><div className="overflow-x-auto"><Table>{children}</Table></div>{loading ? <EmptyState title={`Loading ${title.toLowerCase()}`} description="Fetching payroll rows." /> : empty ? <EmptyState title={emptyTitle} description="Use the available actions or adjust filters." /> : null}</Panel></div>;
}
...
export function PayrollDeductionsPage() {
  ...
  return <PayrollTablePageLayout title="Payroll Deductions" description="Manage fixed, variable, one-time, and recurring deductions." error={error} loading={loading} empty={rows.length === 0} emptyTitle="No deductions" filters={<><SearchInput value={search} onChange={setSearch} /><PayrollOrgFilter departments={departments} locations={locations} jobLevels={jobLevels} positions={positions} departmentId={departmentId} locationId={locationId} jobLevelId={jobLevelId} positionId={positionId} onChange={(next) => { setDepartmentId(next.departmentId); setLocationId(next.locationId); setJobLevelId(next.jobLevelId); setPositionId(next.positionId); }} /><SelectField className="h-9 rounded-md border bg-white px-3 text-sm" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option>{["ACTIVE", "INACTIVE", "APPLIED", "CANCELLED"].map((item) => <option key={item} value={item}>{item}</option>)}</SelectField><SelectField className="h-9 rounded-md border bg-white px-3 text-sm" value={type} onChange={(event) => setType(event.target.value)}><option value="">All types</option>{["FIXED", "VARIABLE", "ONE_TIME", "RECURRING"].map((item) => <option key={item} value={item}>{item}</option>)}</SelectField></>} action={canManage ? <Button size="sm" onClick={() => setEditing({ deduction_type: "ONE_TIME", status: "ACTIVE" })}><Plus className="h-4 w-4" /> Create deduction</Button> : null}>
...
export function PayrollSettingsPage() {
  ...
  return <div className="space-y-4">
    <PayrollPageHeader title="Payroll Settings" description="General payroll, bank-loan, pension, payment, and deduction-priority controls.">{canManage ? <Button size="sm" disabled={!moduleEnabled} onClick={() => void save()}>Save settings</Button> : null}</PayrollPageHeader>
    ...
    <Panel className="p-4">
      {!settings ? <EmptyState title="Loading payroll settings" description="Fetching payroll configuration." /> : <ModuleSettingsBody disabled={!moduleEnabled}><div className="space-y-5">
        ...
        <SettingsSection title="Custom Deduction Settings" description="Employer-defined payroll deductions after pension and bank loan priorities, with payslip, self-service, and shortfall controls.">
          ...
          <Field label="Insufficient salary mode"><SelectField disabled={!canManage} className="h-9 w-full rounded-md border bg-white px-3 text-sm" value={settings.custom_deduction_insufficient_salary_mode ?? "WARN_ONLY"} onChange={(event) => update("custom_deduction_insufficient_salary_mode", event.target.value)}>{["WARN_ONLY", "PARTIAL_DEDUCTION", "SKIP_AND_MARK_FAILED", "BLOCK_PAYROLL", "REQUIRE_OVERRIDE"].map((item) => <option key={item} value={item}>{item}</option>)}</SelectField></Field>
        ...
        <SettingsSection title="Deduction Priority" description="JSON order used by calculation foundations.">
          <div className="md:col-span-2 xl:col-span-3">
            <Field label="payroll_deduction_priority_json">
              <TextareaField disabled={!canManage} className="min-h-24 w-full rounded-md border bg-white px-3 py-2 font-mono text-xs" value={settings.payroll_deduction_priority_json ?? ""} onChange={(event) => update("payroll_deduction_priority_json", event.target.value)} />
            </Field>
          </div>
        </SettingsSection>
...
export function PayrollReportsPage() {
  ...
  return <PayrollTablePageLayout title="Payroll Reports" description="Filter and export payroll summaries with the same visible criteria." error={error} loading={loading} empty={rows.length === 0} emptyTitle="No report rows" filters={<><SelectField className="h-9 rounded-md border bg-white px-3 text-sm" value={report} onChange={(event) => setReport(event.target.value)}>{["summary", "department", "location", "advance", "attendance", "leave", "custom-deductions", "custom-deduction-shortfalls", "employee-history", "final-settlement"].map((item) => <option key={item} value={item}>{item}</option>)}</SelectField><SelectField className="h-9 rounded-md border bg-white px-3 text-sm" value={periodId} onChange={(event) => setPeriodId(event.target.value)}><option value="">All periods</option>{periods.map((period) => <option key={period.id} value={period.id}>{period.period_month}/{period.period_year}</option>)}</SelectField><PayrollOrgFilter departments={departments} locations={locations} jobLevels={jobLevels} positions={positions} departmentId={departmentId} locationId={locationId} jobLevelId={jobLevelId} positionId={positionId} onChange={(next) => { setDepartmentId(next.departmentId); setLocationId(next.locationId); setJobLevelId(next.jobLevelId); setPositionId(next.positionId); }} /><SearchInput value={search} onChange={setSearch} /></>} action={canExport ? <Button size="sm" onClick={() => void exportCsv()}><Download className="h-4 w-4" /> Export CSV</Button> : null}>
