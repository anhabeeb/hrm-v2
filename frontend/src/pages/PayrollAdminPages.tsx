...
function Header({ title, description, children }: { title: string; description: string; children?: React.ReactNode }) {
  return <div className="space-y-3"><div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><div className="min-w-0"><h1 className="text-lg font-semibold">{title}</h1><p className="text-sm text-muted-foreground">{description}</p></div><div className="flex flex-wrap gap-2"><AdminHelpLink target="payroll" label="View Payroll Guide" />{children}</div></div><PayrollNav /></div>;
}
...
function EmployeeSelect({ employees, value, onChange }: { employees: Employee[]; value?: string | null; onChange: (value: string) => void }) {
  return <SelectField className="h-9 w-full rounded-md border bg-white px-3 text-sm" value={value ?? ""} onChange={(event) => onChange(event.target.value)}><option value="">Select employee</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.employee_no} - {employee.full_name}</option>)}</SelectField>;
}
...
function DepartmentFilter({ departments, value, onChange }: { departments: OrganizationDepartment[]; value: string; onChange: (value: string) => void }) {
  return <SelectField className="h-9 rounded-md border bg-white px-3 text-sm" value={value} onChange={(event) => onChange(event.target.value)}><option value="">All departments</option>{departments.map((department) => <option key={department.id} value={department.id}>{department.name}</option>)}</SelectField>;
}

function LocationFilter({ locations, value, onChange }: { locations: OrganizationLocation[]; value: string; onChange: (value: string) => void }) {
  return <SelectField className="h-9 rounded-md border bg-white px-3 text-sm" value={value} onChange={(event) => onChange(event.target.value)}><option value="">All locations</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</SelectField>;
}
...
export function PayrollReportsPage() {
  ...
  return <PageShell title="Payroll Reports" description="Filter and export payroll summaries with the same visible criteria." error={error} loading={loading} empty={rows.length === 0} emptyTitle="No report rows" filters={<><SelectField className="h-9 rounded-md border bg-white px-3 text-sm" value={report} onChange={(event) => setReport(event.target.value)}>{["summary", "department", "location", "advance", "attendance", "leave", "custom-deductions", "custom-deduction-shortfalls", "employee-history", "final-settlement"].map((item) => <option key={item} value={item}>{item}</option>)}</SelectField><SelectField className="h-9 rounded-md border bg-white px-3 text-sm" value={periodId} onChange={(event) => setPeriodId(event.target.value)}><option value="">All periods</option>{periods.map((period) => <option key={period.id} value={period.id}>{period.period_month}/{period.period_year}</option>)}</SelectField><DepartmentFilter departments={departments} value={departmentId} onChange={setDepartmentId} /><LocationFilter locations={locations} value={locationId} onChange={setLocationId} /><SearchInput value={search} onChange={setSearch} /></>} action={canExport ? <Button size="sm" onClick={() => void exportCsv()}><Download className="h-4 w-4" /> Export CSV</Button> : null}>
