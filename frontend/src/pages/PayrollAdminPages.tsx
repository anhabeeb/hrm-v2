...
function PayrollTablePageLayout({ title, description, error, loading, empty, emptyTitle, filters, onReset, action, children }: { title: string; description: string; error: string | null; loading: boolean; empty: boolean; emptyTitle: string; filters: React.ReactNode; onReset?: () => void; action?: React.ReactNode; children: React.ReactNode }) {
  return <div className="space-y-4"><PayrollPageHeader title={title} description={description}>{action}</PayrollPageHeader><ErrorMessage error={error} /><Panel className="overflow-hidden"><div className="border-b p-3"><StandardFilterBar className="border-0 shadow-none">{filters}{onReset ? <FilterResetButton onReset={onReset} /> : null}</StandardFilterBar></div><div className="overflow-x-auto"><Table>{children}</Table></div>{loading ? <EmptyState title={`Loading ${title.toLowerCase()}`} description="Fetching payroll rows." /> : empty ? <EmptyState title={emptyTitle} description="Use the available actions or adjust filters." /> : null}</Panel></div>;
}
...
export function PayrollAdvancesPage() {
  ...
  return <PayrollTablePageLayout title="Payroll Advances" description="Track employee advances by department, location, status, and payment date." error={error} loading={loading} empty={rows.length === 0} emptyTitle="No advances" onReset={() => { setSearch(""); setDepartmentId(""); setLocationId(""); setJobLevelId(""); setPositionId(""); setStatus(""); setFrom(""); setTo(""); }} filters={<><SearchInput value={search} onChange={setSearch} /><SelectField className="h-9 rounded-md border bg-white px-3 text-sm" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option>{["REQUESTED", "APPROVED", "DEDUCTED", "CANCELLED"].map((item) => <option key={item} value={item}>{item}</option>)}</SelectField><StandardDateRangeFilter value={paymentDateRange} onChange={(range) => { setFrom(range.from ?? ""); setTo(range.to ?? ""); }} label="Payment Date Range" /><MoreFiltersSheet title="Advance filters" onReset={() => { setDepartmentId(""); setLocationId(""); setJobLevelId(""); setPositionId(""); }}><FilterSection title="Organization"><PayrollOrgFilter departments={departments} locations={locations} jobLevels={jobLevels} positions={positions} departmentId={departmentId} locationId={locationId} jobLevelId={jobLevelId} positionId={positionId} onChange={(next) => { setDepartmentId(next.departmentId); setLocationId(next.locationId); setJobLevelId(next.jobLevelId); setPositionId(next.positionId); }} /></FilterSection></MoreFiltersSheet></>} action={canManage ? <Button size="sm" onClick={() => setEditing({ status: "REQUESTED", payment_date: new Date().toISOString().slice(0, 10) })}><Plus className="h-4 w-4" /> Create advance</Button> : null}>
...
export function PayrollComponentsPage() {
  ...
  return <PayrollTablePageLayout title="Payroll Components" description="Manage Payroll Core earning and deduction components used in result line items." error={error} loading={loading} empty={filtered.length === 0} emptyTitle="No components" onReset={() => setSearch("")} filters={<SearchInput value={search} onChange={setSearch} placeholder="Search code/name/category" />} action={canManage ? <Button size="sm" onClick={() => setEditing({ type: "ALLOWANCE", calculation_type: "FIXED_AMOUNT", is_active: true, sort_order: 100 })}><Plus className="h-4 w-4" /> Create component</Button> : null}>
...
export function PayrollDeductionsPage() {
  ...
  return <PayrollTablePageLayout title="Payroll Deductions" description="Manage fixed, variable, one-time, and recurring deductions." error={error} loading={loading} empty={rows.length === 0} emptyTitle="No deductions" onReset={() => { setSearch(""); setDepartmentId(""); setLocationId(""); setJobLevelId(""); setPositionId(""); setStatus(""); setType(""); }} filters={<><SearchInput value={search} onChange={setSearch} /><SelectField className="h-9 rounded-md border bg-white px-3 text-sm" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option>{["ACTIVE", "INACTIVE", "APPLIED", "CANCELLED"].map((item) => <option key={item} value={item}>{item}</option>)}</SelectField><SelectField className="h-9 rounded-md border bg-white px-3 text-sm" value={type} onChange={(event) => setType(event.target.value)}><option value="">All types</option>{["FIXED", "VARIABLE", "ONE_TIME", "RECURRING"].map((item) => <option key={item} value={item}>{item}</option>)}</SelectField><MoreFiltersSheet title="Deduction filters" onReset={() => { setDepartmentId(""); setLocationId(""); setJobLevelId(""); setPositionId(""); }}><FilterSection title="Organization"><PayrollOrgFilter departments={departments} locations={locations} jobLevels={jobLevels} positions={positions} departmentId={departmentId} locationId={locationId} jobLevelId={jobLevelId} positionId={positionId} onChange={(next) => { setDepartmentId(next.departmentId); setLocationId(next.locationId); setJobLevelId(next.jobLevelId); setPositionId(next.positionId); }} /></FilterSection></MoreFiltersSheet></>} action={canManage ? <Button size="sm" onClick={() => setEditing({ deduction_type: "ONE_TIME", status: "ACTIVE" })}><Plus className="h-4 w-4" /> Create deduction</Button> : null}>
...
export function PayrollAdjustmentsPage() {
  ...
  return <PayrollTablePageLayout title="Payroll Adjustments" description="Manage manual earning and deduction adjustments." error={error} loading={loading} empty={rows.length === 0} emptyTitle="No adjustments" onReset={() => setSearch("")} filters={<SearchInput value={search} onChange={setSearch} />} action={canManage ? <Button size="sm" onClick={() => setEditing({ adjustment_type: "EARNING", status: "DRAFT" })}><Plus className="h-4 w-4" /> Create adjustment</Button> : null}>
...
export function PayrollReportsPage() {
  ...
  return <PayrollTablePageLayout title="Payroll Reports" description="Filter and export payroll summaries with the same visible criteria." error={error} loading={loading} empty={rows.length === 0} emptyTitle="No report rows" onReset={() => { setReport("summary"); setPeriodId(""); setDepartmentId(""); setLocationId(""); setJobLevelId(""); setPositionId(""); setSearch(""); }} filters={<><SearchInput value={search} onChange={setSearch} /><SelectField className="h-9 rounded-md border bg-white px-3 text-sm" value={report} onChange={(event) => setReport(event.target.value)}>{["summary", "department", "location", "advance", "attendance", "leave", "custom-deductions", "custom-deduction-shortfalls", "employee-history", "final-settlement"].map((item) => <option key={item} value={item}>{item}</option>)}</SelectField><SelectField className="h-9 rounded-md border bg-white px-3 text-sm" value={periodId} onChange={(event) => setPeriodId(event.target.value)}><option value="">All periods</option>{periods.map((period) => <option key={period.id} value={period.id}>{period.period_month}/{period.period_year}</option>)}</SelectField><MoreFiltersSheet title="Payroll report filters" onReset={() => { setDepartmentId(""); setLocationId(""); setJobLevelId(""); setPositionId(""); }}><FilterSection title="Organization"><PayrollOrgFilter departments={departments} locations={locations} jobLevels={jobLevels} positions={positions} departmentId={departmentId} locationId={locationId} jobLevelId={jobLevelId} positionId={positionId} onChange={(next) => { setDepartmentId(next.departmentId); setLocationId(next.locationId); setJobLevelId(next.jobLevelId); setPositionId(next.positionId); }} /></FilterSection></MoreFiltersSheet></>} action={canExport ? <Button size="sm" onClick={() => void exportCsv()}><Download className="h-4 w-4" /> Export CSV</Button> : null}>
