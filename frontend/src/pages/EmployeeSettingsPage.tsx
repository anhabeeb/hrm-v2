    ...
    <div className="space-y-4">
      ...
      <Panel className="overflow-hidden">
        <div className="flex border-b">
          <button className={`h-11 border-b-2 px-4 text-sm font-medium ${tab === "statuses" ? "border-primary" : "border-transparent text-muted-foreground"}`} onClick={() => setTab("statuses")}>Employee Statuses</button>
          <button className={`h-11 border-b-2 px-4 text-sm font-medium ${tab === "numbering" ? "border-primary" : "border-transparent text-muted-foreground"}`} onClick={() => setTab("numbering")}>Employee Numbering</button>
        </div>
        <div className="p-4">
          {tab === "statuses" ? (
...
function NumberingForm({ numbering, preview, canManage, onChange, onSave }: { numbering: EmployeeNumberSettings; preview: string; canManage: boolean; onChange: (value: EmployeeNumberSettings) => void; onSave: () => void }) {
  ...
  return <div className="grid gap-4 lg:grid-cols-3"><div className="space-y-3 lg:col-span-2"><Field label="Prefix" value={numbering.prefix} disabled={!canManage} onChange={(v) => update("prefix", v)} /><Field label="Separator" value={numbering.separator} disabled={!canManage} onChange={(v) => update("separator", v)} /><Field label="Sequence padding" value={String(numbering.sequence_padding)} disabled={!canManage} onChange={(v) => update("sequence_padding", Number(v))} /><Field label="Next sequence" value={String(numbering.next_sequence)} disabled={!canManage} onChange={(v) => update("next_sequence", Number(v))} /><label className="flex items-center gap-2 text-sm"><input type="checkbox" disabled={!canManage} checked={numbering.include_year} onChange={(e) => update("include_year", e.target.checked)} /> Include year</label><label className="flex items-center gap-2 text-sm"><input type="checkbox" disabled={!canManage} checked={numbering.include_location_code} onChange={(e) => update("include_location_code", e.target.checked)} /> Include location code</label><label className="flex items-center gap-2 text-sm"><input type="checkbox" disabled={!canManage} checked={numbering.include_department_code} onChange={(e) => update("include_department_code", e.target.checked)} /> Include department code</label><label className="flex items-center gap-2 text-sm"><input type="checkbox" disabled={!canManage} checked={numbering.allow_manual_override} onChange={(e) => update("allow_manual_override", e.target.checked)} /> Allow manual override</label>{canManage ? <Button size="sm" onClick={onSave}><Save className="h-4 w-4" /> Save numbering</Button> : null}</div><div className="rounded-md border p-3"><p className="text-xs text-muted-foreground">Preview</p><p className="mt-1 font-mono text-lg font-semibold">{preview}</p><p className="mt-3 text-xs text-muted-foreground">Employee numbers must remain unique. Location and department codes are included when configured and supplied.</p></div></div>;
}
