...
function Header({ title, description }: { title: string; description: string }) {
  return <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><div><h1 className="text-lg font-semibold">{title}</h1><p className="text-sm text-muted-foreground">{description}</p></div><PayrollNav /></div>;
}
              ...
              {rows.map((row: PayrollPaymentRegister) => {
                ...
                return (
                  <TableRow key={row.id}>
                    ...
                    <TableCell><div className="font-medium">{row.employee_name_snapshot}</div><div className="font-mono text-xs text-muted-foreground">{row.employee_number_snapshot}</div></TableCell>
                    ...
                    <TableCell><Badge tone={row.payment_status === "MANUALLY_CONFIRMED_PAID" ? "success" : row.payment_status === "CANCELLED" ? "danger" : "neutral"}>{row.payment_status}</Badge></TableCell>
