...
function Header({ title, description, exportRows, exportColumns }: { title: string; description: string; exportRows?: Record<string, unknown>[]; exportColumns?: string[] }) {
  return <><PageHeader title={title} description={description} actions={exportRows && exportColumns ? <ExportMenu moduleName={title} rows={exportRows} columns={exportColumns} /> : null} /><PayrollNav /></>;
}
