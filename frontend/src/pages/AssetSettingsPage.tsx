...
function CategoryModal({ category, onClose, onSaved }: { category?: AssetCategory; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title={category ? "Edit category" : "Create category"} error={error} onClose={onClose} onSave={save}><Field label="Code" value={form.code ?? ""} onChange={(code) => setForm({ ...form, code })} /><Field label="Name" value={form.name ?? ""} onChange={(name) => setForm({ ...form, name })} /><div className="space-y-1.5"><Label>Type</Label><select className="h-9 w-full rounded-md border bg-white px-3 text-sm" value={form.type ?? "ASSET"} onChange={(event) => setForm({ ...form, type: event.target.value as AssetCategory["type"] })}>{["ASSET","UNIFORM","OTHER"].map((value) => <option key={value} value={value}>{value}</option>)}</select></div><Field label="Sort order" type="number" value={String(form.sort_order ?? 100)} onChange={(sort_order) => setForm({ ...form, sort_order: Number(sort_order) })} /><Field label="Description" value={form.description ?? ""} onChange={(description) => setForm({ ...form, description })} /></Modal>;
}
...
function RuleModal({ rule, categories, onClose, onSaved }: { rule?: AssetDeductionRule; categories: AssetCategory[]; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title={rule ? "Edit deduction rule" : "Create deduction rule"} error={error} onClose={onClose} onSave={save}><div className="space-y-1.5"><Label>Category</Label><select className="h-9 w-full rounded-md border bg-white px-3 text-sm" value={form.category_id ?? ""} onChange={(event) => setForm({ ...form, category_id: event.target.value })}><option value="">Any category</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></div><Field label="Condition" value={String(form.condition_status ?? "")} onChange={(condition_status) => setForm({ ...form, condition_status })} /><Field label="Event type" value={String(form.event_type ?? "")} onChange={(event_type) => setForm({ ...form, event_type })} /><div className="space-y-1.5"><Label>Deduction mode</Label><select className="h-9 w-full rounded-md border bg-white px-3 text-sm" value={form.deduction_mode ?? "REPLACEMENT_COST"} onChange={(event) => setForm({ ...form, deduction_mode: event.target.value })}>{["NONE","FIXED_AMOUNT","REPLACEMENT_COST","PERCENTAGE_OF_COST","CUSTOM"].map((value) => <option key={value} value={value}>{value}</option>)}</select></div><Field label="Amount" type="number" value={String(form.deduction_amount ?? "")} onChange={(value) => setForm({ ...form, deduction_amount: value ? Number(value) : null })} /><Field label="Percent" type="number" value={String(form.deduction_percent ?? "")} onChange={(value) => setForm({ ...form, deduction_percent: value ? Number(value) : null })} /></Modal>;
}
