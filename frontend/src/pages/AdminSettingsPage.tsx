...
export function AdminSettingsPage() {
  ...
  return (
    <PageShell>
      ...
      {active === "audit" ? <div className="space-y-4"><Panel className="flex flex-wrap gap-2 p-4"><Input className="w-48" placeholder="Search" value={auditFilters.search} onChange={(e) => setAuditFilters({ ...auditFilters, search: e.target.value })} /><Input className="w-36" placeholder="Module" value={auditFilters.module} onChange={(e) => setAuditFilters({ ...auditFilters, module: e.target.value })} /><Input className="w-36" placeholder="Action" value={auditFilters.action} onChange={(e) => setAuditFilters({ ...auditFilters, action: e.target.value })} /><Input className="w-40" type="date" value={auditFilters.date_from} onChange={(e) => setAuditFilters({ ...auditFilters, date_from: e.target.value })} /><Input className="w-40" type="date" value={auditFilters.date_to} onChange={(e) => setAuditFilters({ ...auditFilters, date_to: e.target.value })} /><Button size="sm" variant="outline" onClick={() => void load()}>Filter</Button></Panel><RowsTable rows={auditRows} columns={["created_at", "module", "action", "entity_type", "entity_id", "actor_email", "reason", "sensitive", "restricted"]} empty="No audit logs found." /></div> : null}
