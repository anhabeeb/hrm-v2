        ...
        {loading || !settings ? <FormSkeleton fields={9} label="Loading roster settings" /> : (
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <Field label="Default week start day">
              <SelectField disabled={controlsDisabled} className="h-9 w-full rounded-md border bg-white px-3 text-sm" value={settings.default_week_start_day} onChange={(event) => update("default_week_start_day", event.target.value as RosterSettings["default_week_start_day"])}>
                <option value="MONDAY">Monday</option>
                <option value="SUNDAY">Sunday</option>
              </SelectField>
            </Field>
            <Field label="Default shift template">
              <SelectField disabled={controlsDisabled} className="h-9 w-full rounded-md border bg-white px-3 text-sm" value={settings.default_shift_template_id ?? ""} onChange={(event) => update("default_shift_template_id", event.target.value || null)}>
                <option value="">No default</option>
                {templates.map((template) => <option key={template.id} value={template.id}>{template.code} - {template.name}</option>)}
              </SelectField>
            </Field>
            <Toggle disabled={controlsDisabled} label="Allow draft roster editing" checked={Boolean(settings.allow_draft_roster_editing)} onChange={(value) => update("allow_draft_roster_editing", value)} />
            <Toggle disabled={controlsDisabled} label="Require publish before employee visibility" checked={Boolean(settings.require_publish_before_employee_visibility)} onChange={(value) => update("require_publish_before_employee_visibility", value)} />
            <Toggle disabled={controlsDisabled} label="Allow unpublish before lock" checked={Boolean(settings.allow_unpublish_before_lock)} onChange={(value) => update("allow_unpublish_before_lock", value)} />
            <Toggle disabled={controlsDisabled} label="Allow changes after publish" checked={Boolean(settings.allow_changes_after_publish ?? settings.allow_published_roster_edits)} onChange={(value) => update("allow_changes_after_publish", value)} />
            <Toggle disabled={controlsDisabled} label="Require reason after publish" checked={Boolean(settings.require_reason_for_changes_after_publish ?? settings.require_reason_for_published_edits)} onChange={(value) => update("require_reason_for_changes_after_publish", value)} />
            <Toggle disabled={controlsDisabled} label="Allow roster lock" checked={Boolean(settings.allow_roster_lock)} onChange={(value) => update("allow_roster_lock", value)} />
            <Toggle disabled={controlsDisabled} label="Lock after attendance/payroll placeholder" checked={Boolean(settings.lock_roster_after_attendance_payroll_placeholder)} onChange={(value) => update("lock_roster_after_attendance_payroll_placeholder", value)} />
            <Toggle disabled={controlsDisabled} label="Allow shift overlap warnings" checked={Boolean(settings.allow_shift_overlap_warnings)} onChange={(value) => update("allow_shift_overlap_warnings", value)} />
            <Toggle disabled={controlsDisabled} label="Block overlapping shifts by default" checked={Boolean(settings.block_overlapping_shifts_by_default)} onChange={(value) => update("block_overlapping_shifts_by_default", value)} />
            <Toggle disabled={controlsDisabled} label="Allow cross-worksite with permission" checked={Boolean(settings.allow_cross_worksite_assignment_with_permission)} onChange={(value) => update("allow_cross_worksite_assignment_with_permission", value)} />
            <Toggle disabled={controlsDisabled} label="Roster-aware attendance" checked={Boolean(settings.roster_aware_attendance_enabled)} onChange={(value) => update("roster_aware_attendance_enabled", value)} />
            <Toggle disabled={controlsDisabled} label="Roster-aware leave counting" checked={Boolean(settings.roster_aware_leave_counting_enabled)} onChange={(value) => update("roster_aware_leave_counting_enabled", value)} />
            <Toggle disabled={controlsDisabled} label="Employee self-service roster visibility" checked={Boolean(settings.employee_self_service_roster_visibility_enabled)} onChange={(value) => update("employee_self_service_roster_visibility_enabled", value)} />
            <Toggle disabled={controlsDisabled} label="Manager team roster visibility" checked={Boolean(settings.manager_team_roster_visibility_enabled)} onChange={(value) => update("manager_team_roster_visibility_enabled", value)} />
            <Toggle disabled={controlsDisabled} label="Copy previous week enabled" checked={Boolean(settings.copy_previous_week_enabled)} onChange={(value) => update("copy_previous_week_enabled", value)} />
            <Toggle disabled={controlsDisabled} label="Bulk assignment enabled" checked={Boolean(settings.bulk_assignment_enabled)} onChange={(value) => update("bulk_assignment_enabled", value)} />
            <Field label="Default break minutes"><InputField disabled={controlsDisabled} type="number" min="0" value={settings.default_break_minutes ?? 60} onChange={(event) => update("default_break_minutes", Number(event.target.value))} /></Field>
            <Field label="Default expected work minutes"><InputField disabled={controlsDisabled} type="number" min="0" value={settings.default_expected_work_minutes ?? 480} onChange={(event) => update("default_expected_work_minutes", Number(event.target.value))} /></Field>
            <Field label="Default off-day handling">
              <SelectField disabled={controlsDisabled} className="h-9 w-full rounded-md border bg-white px-3 text-sm" value={settings.default_off_day_handling_mode ?? "EXPLICIT_ONLY"} onChange={(event) => update("default_off_day_handling_mode", event.target.value)}>
            ...
            <Field label="Public holiday work assignment">
              <SelectField disabled={controlsDisabled} className="h-9 w-full rounded-md border bg-white px-3 text-sm" value={settings.public_holiday_work_assignment_mode ?? "ALLOW_EXPLICIT_SHIFT"} onChange={(event) => update("public_holiday_work_assignment_mode", event.target.value)}>
            ...
            <Toggle disabled={controlsDisabled} label="Allow published roster edits" checked={Boolean(settings.allow_published_roster_edits)} onChange={(value) => update("allow_published_roster_edits", value)} />
            <Toggle disabled={controlsDisabled} label="Require reason for published edits" checked={Boolean(settings.require_reason_for_published_edits)} onChange={(value) => update("require_reason_for_published_edits", value)} />
            <Toggle disabled={controlsDisabled} label="Show leave on roster" checked={Boolean(settings.show_leave_on_roster)} onChange={(value) => update("show_leave_on_roster", value)} />
            <Toggle disabled={controlsDisabled} label="Show attendance on roster" checked={Boolean(settings.show_attendance_on_roster)} onChange={(value) => update("show_attendance_on_roster", value)} />
            <div className="flex justify-end md:col-span-2 xl:col-span-3">{canManage ? <Button onClick={() => void save()} disabled={saving || !moduleEnabled}><Save className="h-4 w-4" /> Save settings</Button> : null}</div>
          </div>
          </ModuleSettingsBody>
        )}
      </Panel>
