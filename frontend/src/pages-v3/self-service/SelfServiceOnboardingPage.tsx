...
function statusTone(status: string): "success" | "warning" | "danger" | "neutral" {
  if (["COMPLETED", "DONE", "ACTIVE"].includes(status)) return "success";
...
export function SelfServiceOnboardingPage() {
  ...
  useEffect(() => {
    if (!token) return;
    let cancelled = false;
    api.getSelfServiceOnboarding(token).then((result) => {
      if (cancelled) return;
      setCurrent((result.onboarding ?? null) as Row | null);
      setTasks(asRows(result.tasks));
      setEvents(asRows(result.events));
    }).catch((err) => { if (!cancelled) setError(err instanceof ApiError ? err.message : "Unable to load your onboarding case."); }).finally(() => { if (!cancelled) setLoading(false); });
  ...
  return (
    <PageShell constrained={false}>
      <div className="space-y-3.5">
        <p className="text-lg font-medium text-slate-950">My onboarding</p>
          ...
          <>
            <Panel className="p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-medium text-slate-950">Onboarding checklist</p>
                  <p className="mt-0.5 text-[10px] text-muted-foreground">This view is read-only and shows tasks assigned to your onboarding case</p>
                </div>
                {current ? <Badge tone={statusTone(text(current.onboarding_status))}>{humanizeTechnicalLabel(text(current.onboarding_status))}</Badge> : null}
              </div>
              {current ? (
                <div className="mt-3.5 grid grid-cols-2 gap-3.5 rounded-md bg-[#F7F7FB] p-3 sm:grid-cols-3">
                  ...
                  <div><p className="text-[9px] text-muted-foreground">Readiness</p><p className="mt-0.5 text-xs text-slate-950">{humanizeTechnicalLabel(text(current.activation_status))}</p></div>
                ...
                <div className="mt-3.5"><EmptyState title="No onboarding case" description="There is no active onboarding case for your employee profile." /></div>
              )}
            </Panel>
