import { Archive, Download, Eye, Pencil, Plus, Settings2, Upload } from "lucide-react";
...
import { EmployeeCardRow, MenuAction } from "../components/employee/EmployeeCardRow";
...
export function EmployeesPage() {
  ...
  async function saveEmployee(input: EmployeeInput) {
    ...
    try {
      ...
      alerts.showSuccess("Employee updated", "The employee record was saved.");
    } catch (err) {
    ...
    <PageShell constrained={false}>
      <PageHeader
        ...
        actions={
          ...
          {canImport ? (
            <Link to="/settings/admin/imports" className="inline-flex items-center gap-1.5 text-xs font-medium text-muted-foreground hover:text-slate-900">
              <Upload className="h-3.5 w-3.5" /> Import
            </Link>
          ) : null}
          {canExport ? <ExportMenu variant="plain" moduleName="Employees" rows={filtered as unknown as Record<string, unknown>[]} columns={["employee_no", "full_name", "employee_type", "employment_type", "status_key", "joining_date", "user_linked", "linked_user_email"]} filterSummary={activeChips.map((chip) => `${chip.label}: ${String(chip.value)}`)} /> : null}
          {canCreate ? <Button size="sm" onClick={() => setModal({ mode: "create" })}><Plus className="h-4 w-4" /> Add new hire</Button> : null}
          </>
        }
      />
      ...
      <div className="space-y-3">
        <StandardFilterBar
          variant="plain"
          search={<StandardSearchInput value={search} onDebouncedChange={setSearch} placeholder="Search employees..." className="sm:w-[260px]" />}
          reset={<FilterResetButton variant="plain" onReset={resetFilters} />}
          moreFilters={
            <MoreFiltersSheet onReset={resetFilters} triggerVariant="plain">
        ...
        >
          <StandardSelectFilter variant="plain" value={statusId === "all" ? "" : statusId} onValueChange={(value) => setStatusId(value || "all")} allLabel="All statuses" width="status" options={statuses.map((status) => ({ value: status.id, label: status.name }))} />
          <StandardSelectFilter variant="plain" value={departmentId === "all" ? "" : departmentId} onValueChange={(value) => { setDepartmentId(value || "all"); setLevelId("all"); setPositionId("all"); }} allLabel="All departments" width="department" options={activeDepartments.map((department) => ({ value: department.id, label: department.name }))} />
          <StandardSelectFilter variant="plain" value={levelId === "all" ? "" : levelId} onValueChange={(value) => { setLevelId(value || "all"); setPositionId("all"); }} allLabel="All job levels" width="jobLevel" options={filteredJobLevels.map((level) => ({ value: level.id, label: level.name }))} />
        </StandardFilterBar>
          ...
          <div className="flex flex-col gap-2">
            {filtered.map((employee, index) => (
              <EmployeeCardRow
                ...
                overflowActions={
                  (canStatus || canArchive) ? (
                    <>
                      {canStatus ? <MenuAction icon={Settings2} onClick={() => { setStatusModalError(null); setStatusModalEmployee(employee); }}>Change status</MenuAction> : null}
                      {canArchive ? <MenuAction icon={Archive} destructive onClick={() => setArchiveTarget(employee)}>Archive</MenuAction> : null}
                    </>
                  ) : null
                }
              />
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/25 p-4">
      <div className="max-h-[92vh] w-full max-w-4xl overflow-hidden rounded-lg border bg-white shadow-xl">
        <div className="flex items-center justify-between border-b px-4 py-3">
          <div><h2 className="text-sm font-semibold">{props.mode === "create" ? "Add new hire" : "Edit employee"}</h2><p className="text-xs text-muted-foreground">{props.mode === "create" ? "Just enough to create the record. Documents, contract, payroll setup, and the rest happen from the onboarding checklist next." : "Profile photo is managed from Employee 360 after the employee record is saved."}</p></div>
