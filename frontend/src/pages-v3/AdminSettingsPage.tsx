...
import { CheckboxField, PageShell, SelectField, type StandardTabItem } from "../components/ui/page-shell";
...
const TAB_ITEMS: StandardTabItem[] = [
  ...
  { key: "checks", label: "Consistency" },
  { key: "audit", label: "Audit log" },
  { key: "security-events", label: "Security events" },
  { key: "permission-risks", label: "Permission risks" },
  { key: "scope-review", label: "Scope review" },
  { key: "security-settings", label: "Security settings" },
  { key: "cache-sync", label: "Cache & sync" },
  { key: "health", label: "Health" },
  { key: "remote-schema", label: "Remote schema" },
  { key: "data-transfer", label: "Data transfer" },
  { key: "retention", label: "Retention" },
  { key: "export-security", label: "Export controls" },
  { key: "readiness", label: "Readiness" },
  { key: "environment", label: "Environment" },
  { key: "alerts", label: "Admin alerts" },
  { key: "reports", label: "Reports" }
];
const TAB_KEYS = new Set(TAB_ITEMS.map((item) => item.key));
  ...
  return (
    <Panel className="overflow-hidden p-2">
      <CardRowList empty={!rows.length} emptyTitle={empty} emptyDescription="Run the checker or adjust filters to see records.">
        {rows.map((row, index) => (
          <KeyValueCardRow
            key={`${String(row.id ?? row.check_key ?? row.module_key ?? row.finding_key ?? "row")}-${index}`}
...
export function AdminSettingsPage() {
  ...
  const [active, setActive] = useState(TAB_KEYS.has(initialSection) ? initialSection : "hub");
  ...
  async function confirmModuleToggle() {
    ...
    await runAction("Module setting updated.", async () => {
      await api.updateAdminModule(token, String(pendingModule.module_key), {
        ...
        reason: "Admin module control update"
      });
  ...
  const helpTarget = active === "modules" ? "moduleSettings" : active === "cache-sync" ? "cacheTimeout" : active === "retention" ? "backupRetention" : active === "data-transfer" ? "dataImport" : active === "health" ? "performance" : null;

  return (
    <PageShell constrained={false}>
      <div className="flex items-center justify-between">
        <div>
          <p className="text-lg font-medium text-slate-950">Admin settings</p>
          <p className="mt-0.5 text-xs text-muted-foreground">Module controls, consistency checks, security logs, and production readiness guardrails</p>
        </div>
        <div className="flex items-center gap-2">
          {helpTarget ? <AdminHelpLink target={helpTarget} label="View guide" /> : null}
      ...
      <div className="mt-3 flex gap-4">
        <NavRail items={TAB_ITEMS} active={active} onChange={selectTab} className="hidden w-[172px] shrink-0 sm:flex" />
        <div className="min-w-0 flex-1 space-y-3">
          ...
          {active === "data-transfer" ? <Panel className="space-y-3 p-4"><div><h2 className="text-sm font-semibold">Data import/export and deployment readiness</h2><p className="text-xs text-muted-foreground">Import validation, export history, backup guidance, migration guidance, QA, smoke tests, and deployment readiness stay separate from destructive production operations. These tools remain on the classic admin pages for now.</p></div><div className="flex flex-wrap gap-2"><NavLink to="/settings/admin/imports"><Button size="sm" variant="outline">Data Import Center</Button></NavLink><NavLink to="/settings/admin/import-templates"><Button size="sm" variant="outline">Import Templates</Button></NavLink><NavLink to="/settings/admin/exports"><Button size="sm" variant="outline">Export Center</Button></NavLink><NavLink to="/settings/admin/backup-readiness"><Button size="sm" variant="outline">Backup Readiness</Button></NavLink><NavLink to="/settings/admin/migration-readiness"><Button size="sm" variant="outline">Migration Guidance</Button></NavLink><NavLink to="/settings/admin/remote-d1-apply-guide"><Button size="sm" variant="outline">Remote D1 Guide</Button></NavLink><NavLink to="/settings/admin/qa-test-matrix"><Button size="sm" variant="outline">QA Matrix</Button></NavLink><NavLink to="/settings/admin/smoke-tests"><Button size="sm" variant="outline">Smoke Tests</Button></NavLink><NavLink to="/settings/admin/deployment-readiness"><Button size="sm" variant="outline">Deployment Readiness</Button></NavLink><NavLink to="/settings/admin/data-transfer-settings"><Button size="sm" variant="outline">Transfer Settings</Button></NavLink><NavLink to="/settings/admin/backup-retention"><Button size="sm" variant="outline">Backup &amp; Retention</Button></NavLink><NavLink to="/settings/performance"><Button size="sm" variant="outline">Performance Dashboard</Button></NavLink></div></Panel> : null}

          {active === "retention" ? <Panel className="space-y-4 p-4"><div className="grid gap-3 md:grid-cols-3"><label className="grid gap-1 text-sm font-medium">Audit log retention days<Input type="number" value={text(retention.audit_log_retention_days)} onChange={(e) => setRetention({ ...retention, audit_log_retention_days: e.target.value })} /></label><label className="grid gap-1 text-sm font-medium">Security event retention days<Input type="number" value={text(retention.security_event_retention_days)} onChange={(e) => setRetention({ ...retention, security_event_retention_days: e.target.value })} /></label><label className="grid gap-1 text-sm font-medium">Notification retention days<Input type="number" value={text(retention.notification_retention_days)} onChange={(e) => setRetention({ ...retention, notification_retention_days: e.target.value })} /></label></div><div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">Automatic destructive cleanup is not implemented yet. These are future-job settings only.</div><Button size="sm" onClick={() => void saveRetention()}>Save retention settings</Button></Panel> : null}

          {active === "export-security" ? <Panel className="space-y-4 p-4"><div className="grid gap-3 md:grid-cols-2"><label className="grid gap-1 text-sm font-medium">Max export rows<Input type="number" value={text(exportSecurity.max_export_rows)} onChange={(e) => setExportSecurity({ ...exportSecurity, max_export_rows: e.target.value })} /></label><label className="grid gap-1 text-sm font-medium">Max date range days<Input type="number" placeholder="Max date range days" value={text(exportSecurity.max_export_date_range_days)} onChange={(e) => setExportSecurity({ ...exportSecurity, max_export_date_range_days: e.target.value })} /></label></div><div className="grid gap-3 text-sm md:grid-cols-2 xl:grid-cols-4"><SettingsToggle label="CSV export" checked={boolInput(exportSecurity.csv_export_enabled)} onChange={(checked) => setExportSecurity({ ...exportSecurity, csv_export_enabled: checked })} /><SettingsToggle label="JSON export" checked={boolInput(exportSecurity.json_export_enabled)} onChange={(checked) => setExportSecurity({ ...exportSecurity, json_export_enabled: checked })} /><SettingsToggle label="Sensitive export reason" checked={boolInput(exportSecurity.sensitive_export_requires_reason)} onChange={(checked) => setExportSecurity({ ...exportSecurity, sensitive_export_requires_reason: checked })} /><SettingsToggle label="Sensitive export audit" checked={boolInput(exportSecurity.sensitive_export_audit_enabled)} onChange={(checked) => setExportSecurity({ ...exportSecurity, sensitive_export_audit_enabled: checked })} /></div><Button size="sm" onClick={() => void saveExportSecurity()}>Save export controls</Button></Panel> : null}
