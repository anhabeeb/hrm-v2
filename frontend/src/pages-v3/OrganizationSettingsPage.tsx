...
import { PageShell, SelectField as UiSelectField, type StandardTabItem } from "../components/ui/page-shell";
import type {
  CompanyInput,
  DepartmentInput,
  JobLevelInput,
    ...
    return (
      <PageShell constrained={false}>
        <div className="space-y-3.5">
          <p className="text-lg font-medium text-slate-950">Organization settings</p>
          <Panel className="p-4"><EmptyState title="Organization settings unavailable" description="Your account needs organization.view permission to open this section." /></Panel>
        </div>
      </PageShell>
    );
  }
  ...
  return (
    <PageShell constrained={false}>
      <div className="flex items-center justify-between">
        <div>
          <p className="text-lg font-medium text-slate-950">Organization settings</p>
          <p className="mt-0.5 text-xs text-muted-foreground">Company structure and master data used by employee profiles</p>
        </div>
        <Button variant="outline" size="sm" onClick={() => void loadOrganization()}><RotateCcw className="h-4 w-4" /> Refresh</Button>
      </div>

      {error ? <div className="mt-3 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{error}</div> : null}

      <div className="mt-3 flex gap-4">
        <NavRail items={TAB_ITEMS} active={activeTab} onChange={(key) => setActiveTab(key as TabKey)} className="hidden w-[172px] shrink-0 sm:flex" />
